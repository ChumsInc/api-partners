...
import { AMAZON_SC_AWSAccessKeyId, AMAZON_SC_MarketplaceId, AMAZON_SC_MWSAuthToken, AMAZON_SC_SellerId, AMAZON_SC_SignatureMethod, AMAZON_SC_SignatureVersion, contentMD5, parseXML, toISO8601, } from './config.js';
import { loadXML, XML_CANCEL_REASON, XML_ENVELOPE, XML_MESSAGE, XML_ORDER_ACK, XML_ORDER_ACK_ITEM, XML_ORDER_FULFILLMENT, XML_ORDER_FULFILLMENT_ITEM } from './templates/index.js';
...
export async function ListOrders(parameters = {}) {
    try {
...
export const doListOrders = async (req, res) => {
    const format = req.params.format ?? 'xml';
    const sage = req.query.sage ?? false;
    const parameters = {
        OrderStatus: [
            'Unshipped',
            'PartiallyShipped',
...
export const doLoadOrderFromDB = async (req, res) => {
    try {
        ...
        if (isListOrdersXMLResponse(json)) {
            const salesOrders = json.ListOrdersResponse?.ListOrdersResult[0]?.Orders[0]?.Order.map((azso) => parseAmazonOrder(azso));
...
export const doGetOrder = async (req, res) => {
    try {
...
export const doListOrderItems = async (req, res) => {
    try {
        const AmazonOrderId = req.params.AmazonOrderId;
...
async function mapToItemCode(item) {
    try {
...
const parseXMLItems = async ({ response }) => {
    try {
        const jsonItems = await parseXML(response);
...
export const doSubmitFeed_OrderAcknowledgement = async (req, res) => {
    try {
        const AmazonOrderId = req.params.AmazonOrderId;
...
export const doSubmitFeed_OrderFulfillment = async (req, res) => {
    try {
        const AmazonOrderId = req.params.AmazonOrderId;
...
export const getOneStepOrder = async (req, res) => {
    try {
        const AmazonOrderId = req.params.AmazonOrderId;
