...
export async function parseFile(filename, removeUpload = true) {
    try {
 ...
 */
export function getMappedField(line, mapping, field, defaultCSVField) {
    const [map] = mapping
        .filter(map => map.MapField === field)
        .map(map => {
...
async function convertToOrder(lines) {
    try {
        ...
        const unitsOfMeasure = await loadItemUnits({ ItemCodes });
        ...
        const SalesOrder = { ...so, ...updateCustomHeader(customer) };
        return {
            SalesOrder,
            mapping,
            customer,
...
export const testCSVFile = async (req, res) => {
    try {
