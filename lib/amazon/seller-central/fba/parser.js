...
function parseSettlement(rows) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const [header] = rows;
            const startDate = (0, date_fns_1.parseJSON)((header === null || header === void 0 ? void 0 : header.settlementStartDate) || '').toISOString();
            const endDate = (0, date_fns_1.parseJSON)((header === null || header === void 0 ? void 0 : header.settlementEndDate) || '').toISOString();
            ...
            rows.filter(row => row.fulfillmentId === 'AFN')
                ...
                if (!itemMap[row.sku]) {
                    order[row.orderItemCode] = Object.assign(Object.assign({}, defaultRow), { orderId: row.orderId, postedDateTime: row.postedDateTime || '', itemCode: `Error: unable to map ${row.sku}`, warehouseCode: 'N/A' });
                    return;
                }
                const item = itemMap[row.sku];
                const orderItem = whseItem(itemMap[row.sku]);
                if (!order[orderItem]) {
                    order[orderItem] = Object.assign(Object.assign({}, defaultRow), { itemCode: item.itemCode, warehouseCode: item.warehouseCode });
                }
            ...
            rows.filter(row => row.fulfillmentId === 'AFN')
                ...
                if (!charges[key]) {
                    charges[key] = Object.assign(Object.assign({}, defaultCharge), { key, glAccount: GLAccounts[key] || '', salesOrderNo: afnKey, amountType: row.amountType, amountDescription: row.amountDescription });
                }
            ...
            rows.filter(row => row.fulfillmentId === 'MFN')
                ...
                const key = `${row.fulfillmentId}:${(0, camelcase_1.default)(row.amountType)}:${(0, camelcase_1.default)(row.amountDescription)}`;
                if (!charges[key]) {
                    charges[key] = Object.assign(Object.assign({}, defaultCharge), { key, glAccount: GLAccounts[key] || '', salesOrderNo: mfnKey, amountType: row.amountType, amountDescription: row.amountDescription });
                }
                charges[key].amount += Number(row.amount);
            ...
            rows.filter(row => row.transactionType !== 'Order')
                ...
                const key = `${row.fulfillmentId}:${(0, camelcase_1.default)(row.amountType)}:${(0, camelcase_1.default)(row.amountDescription)}`;
                if (!charges[key]) {
                    charges[key] = Object.assign(Object.assign({}, defaultCharge), { key, glAccount: GLAccounts[key] || '', salesOrderNo: ascKey, amountType: row.amountType, amountDescription: row.amountDescription });
                }
                charges[key].amount += Number(row.amount);
            });
