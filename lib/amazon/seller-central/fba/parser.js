    ...
    try {
        ...
        rows.filter(row => row.fulfillmentId === 'AFN' && ['Order', 'Refund'].includes(row.transactionType || ''))
            ...
            if (row.amountType === 'ItemPrice' && row.amountDescription === 'Principal') {
                order[sku].quantityPurchased = new decimal_js_1.default(order[sku].quantityPurchased)
                    .add(row.transactionType === 'Refund' ? -1 : (row.quantityPurchased || 0));
            }
            order[sku].extendedUnitPrice = new decimal_js_1.default(order[sku].extendedUnitPrice).add(row.amount || 0);
            order[sku].unitPrice = new decimal_js_1.default(order[sku].quantityPurchased).equals(0)
                ? new decimal_js_1.default(0)
                : new decimal_js_1.default(order[sku].extendedUnitPrice).dividedBy(order[sku].quantityPurchased);
        });
        ...
        rows.filter(row => row.fulfillmentId === 'AFN')
            ...
            charges[key].amount = new decimal_js_1.default(charges[key].amount).add(row.amount || 0);
        });
