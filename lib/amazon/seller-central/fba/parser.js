...
exports.parseSettlementSalesOrder = exports.parseSettlementCharges = exports.parseSettlementBaseData = exports.parseSettlement = exports.buildOrderLines = exports.buildTotals = exports.updateFBMOrders = exports.parseTextFile = void 0;
...
const defaultOrderRow = {
    ...
    warehouseCode: '',
    itemCodeDesc: null,
    extendedUnitPrice: new decimal_js_1.default(0).toString(),
    quantityPurchased: new decimal_js_1.default(0).toString(),
    unitPrice: new decimal_js_1.default(0).toString(),
    orderType: null,
    settlementRow: [],
};
const defaultChargeRow = {
    ...
    amountDescription: '',
    glAccount: '',
    amount: new decimal_js_1.default(0).toString(),
    settlementRow: [],
};
...
async function buildItemMap(rows) {
    try {
        ...
        const unmappedItems = await (0, db_handler_1.loadAMZItemMap)(lookupItems);
...
async function buildCharges(rows, glAccounts) {
    try {
        const charges = {};
        const chargeKey = (row) => {
            return `${row.fulfillmentId}:${row.transactionType || ''}:${(0, camelcase_1.default)(row.amountType ?? '')}:${(0, camelcase_1.default)(row.amountDescription ?? '')}`;
        };
        const altChargeKey = (row) => {
            return `${row.fulfillmentId}:${(0, camelcase_1.default)(row.amountType ?? '')}:${(0, camelcase_1.default)(row.amountDescription ?? '')}`;
        };
        ...
        afnRows.forEach(row => {
            ...
            charges[key].amount = new decimal_js_1.default(charges[key].amount).add(row.amount || 0).toString();
...
async function updateFBMOrders(rows) {
    try {
        ...
        rows.filter(row => row.fulfillmentId === 'MFN')
            ...
            fbmOrders
                ...
                .forEach(so => {
                so.settlementTotal = new decimal_js_1.default(so.settlementTotal).add(row.amount || 0).toString();
            });
        });
        return fbmOrders;
    }
...
exports.updateFBMOrders = updateFBMOrders;
async function buildTotals(rows) {
    try {
        const totals = {
            fba: '0',
            fbaRefund: '0',
        ...
        totals.fba = rows.filter(row => row.fulfillmentId === 'AFN')
            ...
            .reduce((pv, row) => pv.add(row.amount || 0), new decimal_js_1.default(0)).toString();
...
async function buildOrderLines(rows, itemMap) {
    try {
        const order = {};
        rows
            .filter(row => {
            ...
            if (['ItemPrice'].includes(row.amountType ?? '') && row.amountDescription === 'Principal') {
                order[orderKey].quantityPurchased = new decimal_js_1.default(order[orderKey].quantityPurchased)
                    .add(row.transactionType === 'Refund' ? -1 : (row.quantityPurchased || 0))
                    .toString();
            }
            order[orderKey].extendedUnitPrice = new decimal_js_1.default(order[orderKey].extendedUnitPrice).add(row.amount || 0).toString();
            order[orderKey].unitPrice = new decimal_js_1.default(order[orderKey].quantityPurchased).equals(0)
                ? new decimal_js_1.default(0).toString()
                : new decimal_js_1.default(order[orderKey].extendedUnitPrice).dividedBy(order[orderKey].quantityPurchased).toString();
        });
        return order;
    }
    catch (err) {
...
exports.buildOrderLines = buildOrderLines;
...
async function parseSettlementBaseData(rows) {
    try {
        const glAccounts = await (0, db_handler_1.loadGLMap)();
        const [header] = rows;
        const startDate = (0, date_fns_1.parseJSON)(header?.settlementStartDate || '').toISOString();
...
exports.parseSettlementBaseData = parseSettlementBaseData;
async function parseSettlementCharges(rows) {
    try {
        const glAccounts = await (0, db_handler_1.loadGLMap)();
        const charges = await buildCharges(rows, glAccounts);
        return {
...
exports.parseSettlementCharges = parseSettlementCharges;
async function parseSettlementSalesOrder(rows) {
    try {
        const itemMap = await buildItemMap(rows);
...
exports.parseSettlementSalesOrder = parseSettlementSalesOrder;
