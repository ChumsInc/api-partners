import {ParsedCSV, WalmartCSVRow} from "./wm-types";
...
async function parseUpload(req: Request, userId: number): Promise<unknown[]> {
    try {
        const path: FormidableFile = await handleUpload(req);
        debug('parseUpload()', path.filepath);
        const parsed: WalmartCSVRow[] = await csvParser().fromFile(path.filepath);
        await unlink(path.filepath);
