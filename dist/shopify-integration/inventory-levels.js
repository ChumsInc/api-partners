    ...
    try {
        const query = `
            SELECT l.location_id,
                   l.inventory_item_id,
                   GREATEST(FLOOR(IFNULL(a.QuantityAvailable, 0)), 0) AS available,
                   a.ItemCode
            ...
            WHERE a.WarehouseCode = a.DefaultWarehouseCode
              ...
              AND (IFNULL(:ItemCode, '') = a.ItemCode OR
                   (IFNULL(:ItemCode, '') = '' AND GREATEST(FLOOR(IFNULL(a.QuantityAvailable, 0)), 0) <> l.available)
                )
        `;
...
async function postInventoryLevels({ store, ItemCode }) {
    let inventory_levels = [];
    try {
        debug('postInventoryLevels()', { store, ItemCode });
        const items = await loadInventoryLevelsToChange({ store, ItemCode });
        let url = genAdminApiURL('/inventory_levels/set.json', {}, store);
        for await (const item of items) {
            debug('postInventoryLevels()', item.ItemCode, item.available);
...
async function postInventoryLevel({ ItemCode, store }) {
    try {
        const items = await loadInventoryLevelsToChange({ ItemCode, store });
        debug('postInventoryLevel()', items);
...
const setInventoryLevels = async (req, res) => {
    try {
        ...
        const inventory_levels = await postInventoryLevels({ store, ItemCode });
