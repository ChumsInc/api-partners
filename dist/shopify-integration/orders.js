...
const { mapToSageItem } = require("./non-inventory-items");
...
function parseOrder(order) {
    ...
    sage.orderDetail = order.line_items
    ...
    sage.orderCharges = order.line_items
        ...
        const unit_price = ((stdUnitPrice * qty) - discount) / qty;
        const line_discount = (discount / qty);
        const sageItem = mapToSageItem(item);
        if (!sageItem) {
            return null;
        }
        return {
...
async function saveOrderImport({ id, import_result, order }) {
    ...
    try {
        const query = `INSERT INTO shopify.orders (id, import_result, sage_Company, sage_SalesOrderNo,
                       ...
                       VALUES (:id, :import_result, :sage_Company, :sage_SalesOrderNo, :import_status, :shopify_order,
                               NOW())
                       ON DUPLICATE KEY UPDATE import_result = :import_result,
                                               sage_Company = :sage_Company,
                                               sage_SalesOrderNo = :sage_SalesOrderNo,
...
async function fetchOrders({ status = 'open', since_id, created_at_min, created_at_max } = {}) {
    try {
        ...
        if (created_at_min) {
            options.created_at_min = format(startOfDay(parse(created_at_min, 'y-MM-dd', new Date())), "yyyy-MM-dd'T'HH:mm:ssxxx");
        }
        if (created_at_max) {
            options.created_at_max = format(endOfDay(parse(created_at_max, 'y-MM-dd', new Date())), "yyyy-MM-dd'T'HH:mm:ssxxx");
        }
        ...
        // debug('fetchOrders() #orders', allOrders.length);
